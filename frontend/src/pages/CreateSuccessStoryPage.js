...
const CreateSuccessStoryPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.title.trim() || !formData.content.trim()) {
      setError(t("successStories.fillAllFields"));
      return;
    }
    ...
    try {
      setLoading(true);
      setError("");
      const response = await successStoryService.createStory(formData);
      navigate(`/success-stories/${response.id}`);
    } catch (err) {
      setError(
        err.response?.data?.message ||
          err.message ||
          t("successStories.createError")
      );
  ...
  const categories = [
    { value: "RECOVERY", label: t("successStories.category.RECOVERY") },
    { value: "TREATMENT", label: t("successStories.category.TREATMENT") },
    { value: "SUPPORT", label: t("successStories.category.SUPPORT") },
    { value: "LIFESTYLE", label: t("successStories.category.LIFESTYLE") },
    { value: "INSPIRATION", label: t("successStories.category.INSPIRATION") },
  ];
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl flex-1">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {t("successStories.tags")} ({t("successStories.tagsHint")})
              </label>
              <input
                type="text"
                value={formData.tags}
                ...
                placeholder={t("successStories.tagsPlaceholder")}
                className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              />
            </div>
            ...
            <div className="mb-6">
              <label className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                ...
                {t("successStories.shareAnonymously")}
              </label>
            </div>

            <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                {t("successStories.approvalNote")}
              </p>
            </div>
            ...
            <div className="flex gap-4">
              ...
              >
                {loading
                  ? t("successStories.submitting")
                  : t("successStories.submit")}
              </button>
            </div>
          </form>
        </div>
